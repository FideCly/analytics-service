...
import { GetClientsCountRequestDto, GetPromotionsRankingRequestDto } from "./analytics.dto";
...
export class AppService {
	...
	async getAffluence(payload): Promise<GetAffluenceResponse> {
		try {
			...
			const promotions = shop.promotions.filter((promotion) => promotion.isActive);
			...
			filteredBalances = balances.filter((balance) => {
				return (
					balance.createdAt.toISOString()?.split("T")?.[0] >= payload.startDate &&
	 ...
	 * @returns
	 */
	async getPromotionsRanking(payload: GetPromotionsRankingRequestDto): Promise<GetPromotionsRankingResponse> {
		try {
			const { shopId, startDate, endDate } = payload;
	 ...
	 * @returns
	 */
	async getClientsCount(payload: GetClientsCountRequestDto): Promise<GetClientsCountResponse> {
		try {
			const { shopId, startDate, endDate } = payload;
