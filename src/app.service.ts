...
export class AppService {
	...
	async getAffluence(shopId: number, startDate: string, endDate: string): Promise<GetAffluenceResponse> {
		try {
			...
			if (!shop) {
				return { status: 404, value: null, errors: ["Shop not found"] } as GetAffluenceResponse;
			}
			...
			return { status: 200, value: count, errors: null } as GetAffluenceResponse;
		} catch (error) {
			return { status: 500, value: null, errors: [error] } as GetAffluenceResponse;
		}
	}
	...
	async getPromotionCheckoutsCount(
		...
		try {
			...
			return { status: 200, value: count, errors: null } as GetPromotionCheckoutsCountResponse;
		} catch (error) {
			return { status: 500, value: null, errors: [error] } as GetPromotionCheckoutsCountResponse;
		}
	}
	...
	async getPromotionsRanking(
		...
		try {
			...
			return { status: 500, errors: [error], promotionNames: null, values: null } as GetPromotionsRankingResponse;
		}
	}
	 ...
	 * @returns
	 */
	async getClientsCount(shopId: number, startDate: string, endDate: string): Promise<GetClientsCountResponse> {
		const periodDate = this.convertDate(startDate, endDate);
		...
		try {
			...
			if (!shop) {
				return { status: 404, errors: ["Shop not found"] } as GetClientsCountResponse;
			}
			...
			return { status: 200, errors: null, value: userIds.length } as GetClientsCountResponse;
		} catch (error) {
			return { status: 500, value: null, errors: [error] } as GetClientsCountResponse;
		}
	}
