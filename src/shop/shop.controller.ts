...
import { ANALYTICS_PACKAGE_NAME } from "./shop.pb";
...
export class ShopController {
	...
	async createOrUpdate(@Body() createOrUpdateShopDto: CreateOrUpdateShopDto) {
		...
		if (promotion) {
			return this.service.update(+createOrUpdateShopDto.id, createOrUpdateShopDto);
		}

		return this.service.create(createOrUpdateShopDto, +createOrUpdateShopDto.id);
	}
}
